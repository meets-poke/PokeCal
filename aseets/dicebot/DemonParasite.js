...
(function(Opal) {
  ...
  Opal.add_stubs(['$setPrefixes', '$>=', '$==', '$===', '$>', '$get_urge', '$=~', '$to_i', '$<', '$dp_urge_get', '$roll', '$<=', '$[]', '$-']);
  return (function($base, $super, $parent_nesting) {
    function $DemonParasite(){};
    var self = $DemonParasite = $klass($base, $super, 'DemonParasite', $DemonParasite);

    var def = self.$$proto, $nesting = [self].concat($parent_nesting), TMP_DemonParasite_initialize_1, TMP_DemonParasite_gameName_2, TMP_DemonParasite_gameType_3, TMP_DemonParasite_getHelpMessage_4, TMP_DemonParasite_check_nD6_5, TMP_DemonParasite_rollDiceCommand_6, TMP_DemonParasite_get_urge_7, TMP_DemonParasite_dp_urge_get_8;
    ...
    Opal.defn(self, '$rollDiceCommand', TMP_DemonParasite_rollDiceCommand_6 = function $$rollDiceCommand(command) {
      ...
      if (/(\w)?URGE\s*(\d+)/i['$===']($case)) {return self.$get_urge(command)}
      else {return "1"}})()
    }, TMP_DemonParasite_rollDiceCommand_6.$$arity = 1);
    
    Opal.defn(self, '$get_urge', TMP_DemonParasite_get_urge_7 = function $$get_urge(string) {
      var $a, $b, self = this, initialWord = nil, urgelv = nil, urge_type = nil, $case = nil, urge = nil, dice_now = nil, title = nil, resultText = nil, output = nil;
      ...
      urge_type = 0;
      $case = initialWord;
      if (nil['$===']($case)) {urge_type = 1}
      else if (/n/i['$===']($case)) {urge_type = 2}
      else if (/a/i['$===']($case)) {urge_type = 3}
      else if (/m/i['$===']($case)) {urge_type = 4}
      else if (/u/i['$===']($case)) {urge_type = 5}
      else if (/c/i['$===']($case)) {urge_type = 6}
      else {urge_type = 1};
      if ($truthy(($truthy($a = $rb_lt(urgelv, 1)) ? $a : $rb_gt(urgelv, 5)))) {
        return "衝動段階は1から5です"};
      if ($truthy(urge_type['$=='](0))) {
        return "1"};
      urge = self.$dp_urge_get(urge_type);
      $b = self.$roll(2, 6), $a = Opal.to_ary($b), (dice_now = ($a[0] == null ? nil : $a[0])), $b;
      if ($truthy($rb_le(urge_type, 1))) {
        title = "衝動表"
      } else if ($truthy($rb_le(urge_type, 2))) {
        title = "新衝動表"
      } else if ($truthy($rb_le(urge_type, 3))) {
        title = "誤作動表"
      } else if ($truthy($rb_le(urge_type, 4))) {
        title = "ミュータント衝動表"
      } else if ($truthy($rb_le(urge_type, 5))) {
        title = "鬼御魂(戦闘外)衝動表"
        } else {
        title = "鬼御魂(戦闘中)衝動表"
      };
